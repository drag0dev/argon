...
func IsVideoValid(video *Video) bool {
    if (len(video.FileType) == 0) { return false }
    if (video.FileSize == 0) { return false }
    if (video.CreationTimestamp == 0) { return false }
    if (video.LastChangeTimestamp == 0) { return false }
    return true
}

func IsEpisodeValid(episode *Episode) bool {
    if (episode.EpisodeNumber == 0) { return false }
    if (len(episode.Title) == 0) { return false }
    if (len(episode.Description) == 0) { return false }
    for _, actor := range episode.Actors { if (len(actor) == 0) { return false } }
    for _, director := range episode.Directors { if (len(director) == 0) { return false } }
...
func IsSeasonValid(season *Season) bool {
    ...
    for _, episode := range season.Episodes {
        if (!IsEpisodeValid(&episode)) { return false }
    }
    return true
}
